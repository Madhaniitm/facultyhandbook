...
Please provide a clear, complete version of this question that includes the specific topic being discussed. Output only the complete question, nothing else.`;
  ...
  function speakText(text) {
    ...
    // Split into sentences to avoid cutoff issues with long text
    // Browser speech synthesis has character limits, so we chunk it
    const sentences = cleanText.match(/[^.!?]+[.!?]+/g) || [cleanText];
    let currentSentenceIndex = 0;

    function speakNextChunk() {
      if (currentSentenceIndex >= sentences.length) {
        console.log('[Chatbot] All speech chunks completed');
        currentUtterance = null;
        return;
      }

      // Combine up to 3 sentences per chunk (to avoid too many pauses)
      const chunk = sentences.slice(currentSentenceIndex, currentSentenceIndex + 3).join(' ');
      currentSentenceIndex += 3;

      currentUtterance = new SpeechSynthesisUtterance(chunk);
      currentUtterance.lang = 'en-IN'; // Indian English
      currentUtterance.rate = 1.1; // Slightly faster, natural pace
      ...
      currentUtterance.onend = () => {
        console.log('[Chatbot] Chunk ended, speaking next...');
        speakNextChunk(); // Speak next chunk
      };

      currentUtterance.onerror = (event) => {
        console.error('[Chatbot] Speech error:', event);
        currentUtterance = null;
      };

      console.log('[Chatbot] Speaking chunk:', chunk.substring(0, 50) + '...');
...
${context}`;
  ...
  async function addBotMessage(message, references = []) {
    ...
    // Automatically speak the response (like Siri/Google Assistant)
    speakText(message);
    speakerBtn.classList.add('speaking');
    // Update speaker button when speech ends
    setTimeout(() => {
